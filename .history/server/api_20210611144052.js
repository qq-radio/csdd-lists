...
app.post("/login", function (req, res) {
    models.User
        ...
        .then((user,err) => {
            ...
            console.log('er1000000000000000000000000000000000000011r--------------------------',err);
            const token = jwtSign({ _id: user[0]._id })
            res.send({ 'status': 200, 'message': '登录成功！', 'data': { token: token, _id: user[0]._id } });
        }).catch(err=>{
            console.log('er111111111111111111111111111111111111111111111r--------------------------',err);
        })     
})
...
app.post("/lists", jwtCheck,function (req, res) {
    models.List
        ...
        .then((lists) => {
            ...
            console.log('user lists res -------------', lists);
            res.json(lists)
        })
})
