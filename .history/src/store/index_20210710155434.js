  ...
  mutations: {
    SET_LISTS(state, payload) {
      ...
      if (payload.action == 'save') {
        ...
        state.lists[payload.year].forEach((i,index) => {


          if (payload.item.month < i.month) {
            console.log('index',payload.item.month , '----',i.month);
            console.log('item.month -----', payload.item.month, "item.month ---", item.month);
            state.lists[payload.year].splice(index, 0, payload.item);
            return
          }
        })
      }
