        ...
        <el-form :model="form">
          <el-table ref="table" :data="lists[year]" stripe>
            ...
            <el-table-column prop="operation" :label="$t('action')">
              <template v-slot="scope">
                <template v-if="scope.row.action == 'view'">
                  <el-button size="mini" @click="click_edit(scope.row, scope.$index)">
                    <i class="iconfont icon-17"></i>
                  </el-button>
                  <el-button size="mini" @click="click_delete(scope.row, scope.$index)">
  ...
  methods: {
    ...
    change_language() {
      ...
      switch (this.$i18n.locale) {
        case "cn":
          this.$message("已切换至中文模式 !");
          break;
        case "en":
          this.$message("change language to English successfully !");
          break;
      }
    },
...
@bgColor: rgb(138, 188, 209);
.home {
  position: relative;
  width: 100%;
  height: 100%;
  .head {
    background-color: fade(@bgColor, 40%);
    position: fixed;
    display: flex;
