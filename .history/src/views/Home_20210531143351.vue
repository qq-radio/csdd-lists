...
export default {
  ...
  methods: {
    ...
    resetField(form, index) {
      console.log("001", this.$refs.form.model.datas, index);
    ...
    click_add(item, index) {
      ...
      let date = new Date().getDate() < 10 ? "0" + Number(new Date().getDate()) : new Date().getDate();
      ...
      let temp = JSON.parse(localStorage.getItem("costs"));
      temp.push(itemClone);

      console.log("88888", temp);

      localStorage.setItem("costs", JSON.stringify(temp));
