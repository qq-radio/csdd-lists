    ...
    handle_beforeLeave(tab) {
      ...
      if (this.lists[this.year]) {
        for (let i = 0; i < this.lists[this.year].length; i++) {
          if (this.lists[this.year][i].action == "edit" || this.lists[this.year][i].action == "edit") {
            // this.$alert(this.$t("confirm.save"), this.$t("alert.register_failed_title"), { confirmButtonText: this.$t("btn.sure"), customClass: "infoBox" });
