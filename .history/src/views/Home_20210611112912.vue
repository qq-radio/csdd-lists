  ...
  <div class="home">
    <div class="head">
      ...
      <div class="right">
        ...
        <el-select v-model="yearValue" @change="change_year">
          <el-option v-for="i in yearArr" :key="i.value" :label="i.label" :value="i.value"> </el-option>
        </el-select>
      </div>
    </div>

    <el-tabs v-model="activeTab" type="border-card" @tab-click="click_tab" v-loading="loading" element-loading-text="Loading..." element-loading-spinner="el-icon-loading">
          ...
          <el-table ref="table" :data="lists[year]">
            ...
            <el-table-column prop="realSalary" :label="$t('realSalary')">
              <template v-slot="scope">
                ...
                <template v-else>
                  <el-form-item>
                    <el-input v-model.number="scope.row.realSalary" style="width: 100px"></el-input>
                  </el-form-item>
                </template>
              </template>
            </el-table-column>
...
export default {
  ...
  methods: {
    ...
    change_year() {
      this.CHANGE_YEAR(this.yearValue);
      this.get_datas();
    },
    set_monthArr() {
      this.monthArr = [];
