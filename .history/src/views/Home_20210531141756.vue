  ...
  methods: {
    ...
    getData() {
      if (localStorage.hasOwnProperty("testAAA") == false) {
        ...
        console.log("dfsd");
        localStorage.setItem("testAAA", JSON.stringify(obj));
      }
    ...
    click_add(item, index) {
      ...
      localStorage.setItem("testAAA", JSON.stringify(temp));
    ...
    click_delete(item, index) {
      this.$confirm("确定删除该条数据(ID" + item.id + ")吗?", "提示", {
        ...
        .then(() => {
          ...
          localStorage.setItem("testAAA", JSON.stringify( this.form.datas));
        })
        .catch(() => {});
    },
  },
