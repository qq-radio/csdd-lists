<template>
  <div class="home">
    ...
    <el-button style='position:absolute;top:0;' size="mini" @click="click_add_more()">新增yihang!!!!</el-button>
            ...
            <el-table-column prop="operation" label="操作">
              ...
              <template v-slot="scope">
                ...
                <template v-else-if="scope.row.action == 'add'">
                  ...
                  <el-button size="mini" @click="click_add_save(scope.row, scope.$index)">保存add</el-button>
                  <el-button size="mini" @click="click_add_cancle(scope.row, scope.$index)">取消add</el-button>
                </template>
                <template v-else>
...
export default {
  ...
  methods: {
    ...
    click_add_more() {
      ...
      this.unSubmit[(this.form.datas.length -1)] = {}
      console.log('000',JSON.parse(JSON.stringify( this.unSubmit)));
      
    },
    click_add_save(val, index) {
      // this.form.datas[index]=val
    },
    click_add_cancel() {
      this.form.datas.push(this.form.data);
    },
