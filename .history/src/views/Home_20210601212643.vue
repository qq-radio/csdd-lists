...
export default {
  ...
  methods: {
    ...
    change() {
      this.get_datas(this.year);
    },

    get_datas(year) {
      if (localStorage.hasOwnProperty(year) == false) {
        let obj = [
          {
            month: 1,
            realSalary: "",
      ...
      this.lists[year] = JSON.parse(localStorage.getItem(year));
    },

    click_add() {
      this.lists[year].push(JSON.parse(JSON.stringify(this.newItem)));
    },
    ...
    click_save(item, index) {
      ...
      item.action = "view";
      this.lists[year][index] = item;
      localStorage.setItem(this.year, JSON.stringify(this.lists[year]));
    },

    click_cancle(item, index) {
      this.lists[year][index].action = "view";
      if (this.itemClone[index]) {
        this.itemClone[index].action = "view";
        this.lists[year][index] = this.itemClone[index];
        localStorage.setItem(this.year, JSON.stringify(this.lists[year]));
      }
    },
    ...
    click_delete(item, index) {
      this.$confirm("确定删除第" + index + "数据吗?", "提示", {
        ...
        .then(() => {
          this.lists[year].splice(index, 1);
          localStorage.setItem(this.year, JSON.stringify(this.lists[year]));
        })
        .catch(() => {});
    },
