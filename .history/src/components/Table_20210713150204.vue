      ...
      <el-table ref="table" :data="lists[year]">
        <el-table-column class="month" prop="month" :label="$t('month')">
          <template v-slot="scope">
            ...
            <template v-else>
              <el-form-item>
                <el-select v-model.number="scope.row.month" @click="click_monthArr(scope.$index)">
...
export default {
  ...
  watch: {},
  methods: {
    ...mapMutations(["SET_LISTS", "SET_MONTH_DISABLE"]),
    ...mapActions(["get_datas", "get_monthArr"]),
    ...
    click_add() {
      ...
      if (arr.length !== 0) {
        ...
        if (arr[arr.length - 1].month == 12 || arr[arr.length - 1].month == "") {
          this.newItem.month = "";
        } else {
          this.newItem.month = Number(arr[arr.length - 1].month) + 1;
          this.SET_MONTH_DISABLE({ month: this.newItem.month, disabled: true });
        }
      }
