            ...
            <template v-else>
              <el-form-item>
                <el-select v-model.number="scope.row.month">
                  <el-option v-for="i in monthArr[scope.$index]" :key="i.value" :label="i.label" :value="i.value" :disabled="i.disabled"> </el-option>
                </el-select>
              </el-form-item>
            </template>
          </template>
...
export default {
  ...
  methods: {
    ...
    set_monthArr() {
      ...
      for (let i = 1; i < 13; i++) {
        if (tempArr.indexOf(i) !== -1 || tempArr.indexOf(this.month)) {
          this.monthArr.push({
            value: i,
            label: i,
            disabled: true,
