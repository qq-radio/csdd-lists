<template>
  <div class="graphics">
    ...
    <span class="month" v-show="show.two">{{ $t("graphics.changeMonth") }}</span>
...
export default {
  ...
  methods: {
    ...
    set_graphics_line() {
      this.desc = this.year + "  " + this.$t("graphics.desc_line");
    ...
    set_graphics_pie() {
      this.desc = this.year + "  " + this.$t("graphics.desc_pie");
      let chartDom = document.getElementById("one");
      ...
      let option = {
        ...
        series: [
          {
            name: this.year + this.$t("graphics.year") + this.month + this.$t("graphics.month"),
            type: "pie",
            radius: "50%",
            data: [],
            emphasis: {
    ...
    set_graphics() {
      if (sessionStorage.hasOwnProperty("graphics") == false) {
        sessionStorage.setItem("graphics", JSON.stringify(this.show.active));
      } else {
