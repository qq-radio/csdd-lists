<template>
  <div class="editUser">
    ...
    <div v-if="activeStep == '3'" class="sure">
      ...
      <el-button @click="$router.push({ name: 'home' })" type="primary" size="medium">{{ $t("btn.sure") }}</el-button>
    </div>
  </div>
</template>
  ...
  methods: {
          ...
          this.axios({ method: "post", url: "/api/login", data: this.form_1 })
            .then((res) => {
              ...
              if (res.data.status !== 200) {
                this.$alert("用户名或密码错误", "验证失败", { confirmButtonText: "确定", customClass: "infoBox" });
                return;
              }
    ...
    click_step_2() {
      this.$refs["refForm_2"].validate((valid) => {
        if (valid) {
          this.axios({ method: "post", url: "/api/editUser", data: { username: this.form_1.username, new_password: this.form_2.new_password } })
            .then((res) => {
              if (res.data.status == 200) {
