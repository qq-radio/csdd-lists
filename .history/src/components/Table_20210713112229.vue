...
export default {
  ...
  methods: {
    ...
    click_add() {
      let arr = JSON.parse(JSON.stringify(this.lists[this.year]));
      if (arr.length !== 0) {
        if (arr.length >= 12) {
          this.$message({ message: this.$t("msg.warn"), type: "info" });
          return;
        }
        arr[length - 1].month == 12 || arr[length - 1].month == "" ? (this.newItem.month = "") : (this.newItem.month = Number(arr[length - 1].month) + 1);
      }
      ...
      this.$confirm(this.$t("confirm.delete"), this.$t("confirm.delete_title"), { confirmButtonText: this.$t("btn.sure"), cancelButtonText: this.$t("btn.cancel"), cancelButtonClass: "infoBoxCancel", customClass: "infoBox", type: "warning" })
        .then(() => {
          this.axios
            ...
            .then((res) => {
              this.SET_LISTS({ action: "delete", year: this.year, index: index, month: item.month });
