...
export default {
  ...
  methods: {
    ...
    click_add() {
      ...
      console.log("yyyy", this.lists[this.year][length - 1]);

      this.lists[this.year][length - 1].month == 12 ? "" : (this.newItem.month = Number(this.lists[this.year][length - 1].month) + 1);
          ...
          this.axios
            ...
            .then((res) => {
              ...
              if (res.data.status == 200) {
                item._id = res.data.data._id;
                this.SET_LISTS({ action: "save", year: this.year, index: index, item: JSON.parse(JSON.stringify(item)) });
                this.$message({ message: "编辑成功", type: "info", customClass: "info" });
              }
            })
            .catch(function (err) {});
          break;
        ...
        .then(() => {
          this.axios
            ...
            .then((res) => {
              ...
              this.$message({ message: "删除成功", type: "info", customClass: "info" });
              // this.get_datas({ year: this.year }).then(this.$message({ message: "删除成功", type: "info", customClass: "info" }));
            })
            .catch(function (err) {});
        })
        .catch(() => {});
