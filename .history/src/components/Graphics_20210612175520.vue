...
export default {
  ...
  methods: {
    ...
    get_datas() {
      console.log("555", localStorage.hasOwnProperty(JSON.parse(sessionStorage.getItem("year"))));

      if (localStorage.hasOwnProperty(JSON.parse(sessionStorage.getItem("year"))) == false) {
        this.lists[this.year] = [];
      } else {
        this.lists[this.year] = localStorage.hasOwnProperty(JSON.parse(sessionStorage.getItem("year")));
      }
      this.set_graphics();
    },
  },
